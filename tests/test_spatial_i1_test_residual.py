...
def run_stata_spatial_i1_test_residual(
    tmp_path: Path,
    df: pd.DataFrame,
) -> tuple[float, float]:
    ...
    input_csv = tmp_path / "spatial_i1resid_input.csv"
    output_csv = tmp_path / "spatial_i1resid_output.csv"
...
def test_spatial_i1_test_residual_returns_valid_result() -> None:
...
def test_spatial_i1_test_residual_matches_stata(
    tmp_path: Path,
    chetty_df: pd.DataFrame,
) -> None:
    coords = chetty_df[["lat", "lon"]].to_numpy()
    y = chetty_df["am"].to_numpy()
    X_in = np.column_stack([np.ones(len(chetty_df)), chetty_df["fracblack"].to_numpy()])
    distmat = get_distmat_normalized(coords, latlon=True)
    emat = np.random.default_rng(42).standard_normal((10, NREP))

    py_value = spatial_i1_test_residual(y, X_in, distmat, emat)
    st_LR, st_ha = run_stata_spatial_i1_test_residual(tmp_path, chetty_df)
