...
from spur import spurtransform, lbmgls_matrix
...
def fig_chetty_maps():
    ...
    for ax, data, title in zip(
        ...
        ['AM (standardized)', 'TLFPR (standardized)',
         'AM after LBM-GLS', 'TLFPR after LBM-GLS']
    ):
        # Color by decile
...
def fig_chetty_regressions():
    ...
    ax.set_title(f'Before LBM-GLS\n'
                 fr'$\beta$={reg_level.params[1]:.3f}, '
                 fr'$R^2$={reg_level.rsquared:.3f}, '
    ...
    ax.set_title(f'After LBM-GLS\n'
                 fr'$\beta$={reg_d.params[1]:.3f}, '
                 fr'$R^2$={reg_d.rsquared:.3f}, '
...
def fig_mc_variability():
    ...
    import shutil
    src = Path("D:/UZHechist Dropbox/Joachim Voth/claudecode/spur-python/mc_variability_comparison.png")
    if src.exists():
...
def fig_validation_summary():
    ...
    ax.axvline(1e-6, color='red', linestyle='--', alpha=0.5, label='1e-6 (floating point)')
    ax.axvline(1e-3, color='orange', linestyle='--', alpha=0.5, label='1e-3 (MC noise OK)')
    ax.legend()
...
def fig_mc_convergence():
    ...
    df = pd.read_csv("D:/UZHechist Dropbox/Joachim Voth/claudecode/spur-python/spurtest_data.csv")
