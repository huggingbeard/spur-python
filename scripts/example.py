...
def main():
    ...
    print(
        f"   Longitude range: {df['longitude'].min():.2f} - {df['longitude'].max():.2f}"
    )
    ...
    print(
        f"   outcome:    mean={df['outcome'].mean():.4f}, std={df['outcome'].std():.4f}"
    )
    print(
        f"   nn_outcome: mean={df_nn['nn_outcome'].mean():.4f}, std={df_nn['nn_outcome'].std():.4f}"
    )
    print(
        f"   treatment:    mean={df['treatment'].mean():.4f}, std={df['treatment'].std():.4f}"
    )
    print(
        f"   nn_treatment: mean={df_nn['nn_treatment'].mean():.4f}, std={df_nn['nn_treatment'].std():.4f}"
    )
    ...
    print(
        f"   iso_outcome: mean={df_iso['iso_outcome'].mean():.4f}, std={df_iso['iso_outcome'].std():.4f}"
    )
    ...
    print(
        f"   outcome & nn_outcome:  {np.corrcoef(df_both['outcome'], df_both['nn_outcome'])[0, 1]:.4f}"
    )
    print(
        f"   outcome & iso_outcome: {np.corrcoef(df_both['outcome'], df_both['iso_outcome'])[0, 1]:.4f}"
    )
    print(
        f"   nn_outcome & iso_outcome: {np.corrcoef(df_both['nn_outcome'], df_both['iso_outcome'])[0, 1]:.4f}"
    )
    ...
    print("   Euclidean NN transform applied")
    print(
        f"   euc_outcome: mean={df_xy['euc_outcome'].mean():.4f}, std={df_xy['euc_outcome'].std():.4f}"
    )
