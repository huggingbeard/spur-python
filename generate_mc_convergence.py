...
for seed in seeds:
    for n in nreps:
        ...
        r = spurtest(df, 'i1', 'y_i1', ['lat', 'lon'],
                     q=10, nrep=n, seed=seed)
        results.append({'seed': seed, 'nrep': n,
                        'LR': r.LR, 'ha_param': r.ha_param,
                        'pvalue': r.pvalue})
...
ax.errorbar(means.index, means.values, yerr=stds.values,
            marker='o', capsize=5, lw=2, color='navy')
ax.set_xscale('log')
...
ax.errorbar(means.index, means.values, yerr=stds.values,
            marker='o', capsize=5, lw=2, color='darkred')
ax.set_xscale('log')
ax.set_xlabel('Monte Carlo draws (nrep)')
