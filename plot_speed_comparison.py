...
st = pd.read_csv("D:/UZHechist Dropbox/Joachim Voth/claudecode/spur-python/stata_speed_results.csv")
...
print(cmp[['function', 'n', 'nrep', 'time_sec_py', 'time_sec_st', 'speedup']].to_string(index=False))
...
for i, method in enumerate(methods):
    ...
    ax.bar(x + offset - 0.12, sub['time_sec_py'].values, width=0.1,
           label=f'Py {method}', alpha=0.85)
    ax.bar(x + offset - 0.02, sub['time_sec_st'].values, width=0.1,
           label=f'St {method}', alpha=0.6, hatch='//')
...
for i, method in enumerate(methods):
    sub = tf[tf['method'] == method].sort_values('n')
    offset = (i - 1) * 0.25
    bars = ax.bar(x + offset, sub['speedup'].values, width=0.2,
                  label=f'{method}', alpha=0.85)
...
for i, tt in enumerate(test_types):
    for j, nrep in enumerate(nreps):
        sub = tests[(tests['function'] == tt) & (tests['nrep'] == nrep)].sort_values('n')
...
for i, tt in enumerate(test_types):
    ...
    ax.plot(sub['n'], sub['time_sec_py'], marker='o', lw=2,
            label=f'Python {tt_short}', linestyle='-')
    ax.plot(sub['n'], sub['time_sec_st'], marker='s', lw=2,
            label=f'Stata {tt_short}', linestyle='--')
...
for i, nrep in enumerate([10000, 50000]):
    ...
    ax.bar(x + (i - 0.5) * width - 0.1, sub['time_sec_py'].values,
           width=0.18, label=f'Python nrep={nrep}', alpha=0.85)
    ax.bar(x + (i - 0.5) * width + 0.1, sub['time_sec_st'].values,
           width=0.18, label=f'Stata nrep={nrep}', alpha=0.85, hatch='//')
...
# Summary table
# =============================
summary = cmp.groupby('function').apply(
    lambda g: pd.Series({
